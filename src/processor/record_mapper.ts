import { EnrichedRecord, RecordsRow } from "./types";

// ---------------------------------------------------------------------------
// Business name detection
// ---------------------------------------------------------------------------
const BUSINESS_SUFFIXES =
  /\b(inc|llc|corp|ltd|co|lp|llp|pllc|pc|pa|na|dba|company|corporation|incorporated|limited|associates|group|trust|fund|entity|services|solutions|systems|technologies|tech|industries|enterprises|holdings|management|partners|bank|financial|realty|properties|investments|ventures|consulting|international)\b/i;
...
function isBusinessName(name: string): boolean {
  ...
  if (words.length >= 3 && words.every(w => w === w.toUpperCase() && /^[A-Z]/.test(w))) return true;
  return false;
}
...
function parseName(raw: string): {
  ...
  const name = raw.trim();
  if (!name) return { businessPersonal: "Personal", company: "", firstName: "", lastName: "" };
...
// ---------------------------------------------------------------------------
export const RECORDS_HEADERS: RecordsRow = [
  "Site Id",
  "LienOrReceiveDate",
  "Amount",
  "LeadType",
...
export function mapToRecordsRow(record: EnrichedRecord): RecordsRow {
...
export function mapToRecordsRows(records: EnrichedRecord[]): RecordsRow[] {
